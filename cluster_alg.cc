//
// Copyright (C) 2008
// ClusterAlg simple example for INET (add-on)
// Version 2.0
...
{
    // When ClusterAlg module receives selfmessage (scheduled event)
    ...
    if (msg == event)
    {
        ...
        // Filling the ClusterAlgHello fields
    ...
    else
    {
        for (auto it = forwardList->begin(); it != forwardList->end(); it++)
        {
            if ( (*it)->event == msg )
            {
                EV << "Vou mandar forward do " << (*it)->hello->peekData<ClusterAlgHello>()->getSrcAddress() << endl; // todo
                send((*it)->hello, "ipOut");
                (*it)->hello = nullptr;
                delete (*it);
...
void ClusterAlg::handleMessageWhenUp(cMessage *msg)
{
    ...
    else if (check_and_cast<Packet *>(msg)->getTag<PacketProtocolTag>()->getProtocol() == &Protocol::manet)
    {
        ...
        auto recHello = !isForwardHello ? staticPtrCast<ClusterAlgHello>(check_and_cast<Packet *>(msg)->peekData<ClusterAlgHello>()->dupShared()) : nullptr;
        if (isForwardHello) {
        ...
        if (msg->arrivedOn("ipIn")) {
            ...
            //reads ClusterAlg hello message fields
            Ipv4Address src;
            ...
            if (!isForwardHello) {
                ...
                auto fhpHello = fhp->hello->peekData<ClusterAlgHello>();
                    ...
                    || (entrada_routing != nullptr && (msgsequencenumber>(entrada_routing->getSequencenumber()) || (msgsequencenumber == (entrada_routing->getSequencenumber()) && numHops < (entrada_routing->getMetric())))))
            {
                ...
                if (!isForwardHello) {
                    ...
                    auto forwardHello = staticPtrCast<ClusterAlgHello>(fhp->hello->peekData<ClusterAlgHello>()->dupShared());
